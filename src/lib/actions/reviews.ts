...
export async function addRestaurantReview(restaurantId: string, formData: FormData) {
  ...
  // Recompute community confidence from review safety ratings (1-5 -> 0-100)
...
export async function addProductReview(productId: string, formData: FormData) {
  ...
  const content = String(formData.get("content") || "").trim();
  if (!content) return { error: "Please write a short review" };
  await prisma.productReview.create({ data: { productId, userId: user.id, rating, content } });
  revalidatePath(`/app/products/${productId}`);
  return { ok: true };
}

export async function reportIngredientChange(productId: string, formData: FormData) {
  const user = await requireUser();
  const description = String(formData.get("description") || "").trim();
  if (!description) return { error: "Describe what changed" };
  await prisma.ingredientChangeReport.create({
    data: { productId, userId: user.id, description },
  });
  revalidatePath(`/app/products/${productId}`);
  return { ok: true };
}

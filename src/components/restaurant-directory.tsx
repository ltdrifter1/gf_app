...
export function RestaurantDirectory({
  ...
  function enableNearMe() {
    if (!navigator.geolocation) {
      setGeoError("Location isn't available here");
      return;
    }
    setLocating(true);
    setGeoError("");
  ...
  const filtered = useMemo(() => {
    ...
    if (nearMe) {
      list = list
        .map((r) => ({ ...r, _km: distanceKm(nearMe, { lat: r.lat, lng: r.lng }) }))
        .sort((a, b) => a._km - b._km);
    }

    return list;
  }, [restaurants, query, active, city, nearMe]);
  ...
  const listPanel = (
    <div className="space-y-3">
      <p className="text-sm text-sage-500 dark:text-sage-400">
        ...
        {nearMe ? " near you" : city ? ` in ${city}` : ""}
      </p>
      {filtered.map((r) => {
        const km = nearMe ? distanceKm(nearMe, { lat: r.lat, lng: r.lng }) : null;
        return (
          <Link
            key={r.id}
      ...
      {filtered.length === 0 && (
        <div className="card p-8 text-center text-sage-500">
          No restaurants match those filters. Add one below?
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <AddRestaurantForm defaultCity={defaultCity} />
      </div>
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
          ...
          {nearMe && (
            <button
              ...
              className="chip shrink-0 border border-transparent bg-white/60 text-sage-600 dark:bg-white/5"
            >
              Clear location
            </button>
          )}

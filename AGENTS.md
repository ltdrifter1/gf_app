...
| Profile Studio | `/app/profile/studio` |
...
- **Account** — Forgot password (Resend when `RESEND_API_KEY` is set; otherwise a one-hour recovery link is shown in the UI). ZIP export, delete account, notification prefs + quiet hours.
- **Profile Studio** — `/app/profile/studio` skins, cover/wallpaper, module order, explicit Top 8, guestbook.
- **Uploads / scan** — Photos via Vercel Blob or `public/uploads`. HTTPS-only `<img>` URLs. OCR text shown. Miss reports. 90-day scan retention unless kept.
...
- Optional email: `RESEND_API_KEY` + `EMAIL_FROM` (password reset emails). Without the key, `/forgot` still issues a one-hour token and shows the link in the UI so recovery is never a dead end.
